...
import { noSubmit } from '../../js/common/input/form.js';

window.nextStep = function nextStep(event) {
...
window.awaitPaymentChoice = function awaitPaymentChoice() {
  ...
  radios.forEach((radio) => {
    radio.addEventListener('change', () => {
      const buttons = Array.from(
        ...
        .querySelector('.container-button-group');
      ...
      if (selected == creditCard) {
        ...
        buttons.style.marginTop = `${
          window.innerHeight - (formHeight - 135)
        }px`;
      } else {
...
window.adjustButtonsPosition = function adjustButtonsPosition() {
