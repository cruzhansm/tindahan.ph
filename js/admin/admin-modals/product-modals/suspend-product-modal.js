...
import { deleteProdModal } from '../../db-methods/retrieve.js'
...
window.showDeleteModal = function showDeleteModal(selectedModal, prodId, index) {
  const modal = new bootstrap.Modal(selectedModal);
...
window.suspendedProdModal = function suspendedProdModal(selectedModal, prodId, index) {
  const modal = bootstrap.Modal.getInstance(selectedModal);
  const listing = LISTINGS[index];
