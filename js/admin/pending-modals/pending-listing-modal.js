...
window.listingShowModal = function listingShowModal(
  selectedModal,
  applicationId,
  index
) {
  ...
  modal.show();
  const application = LISTING_APPLICATIONS[index];
  console.log(index);
...
window.listingApprovedModal = function listingApprovedModal(
  selectedModal,
  applicationId
) {
  const modal = bootstrap.Modal.getInstance(selectedModal);

  approveListing(applicationId, modal);
};
...
window.listingRejectedModal = function listingRejectedModal(
  selectedModal,
  index
) {
  const modal = bootstrap.Modal.getInstance(selectedModal);
